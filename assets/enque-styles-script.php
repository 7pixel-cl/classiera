...
function classiera_scripts_styles(){
	...
	wp_enqueue_script('mixpanel', get_template_directory_uri() . '/js/mixpanel.js', 'jquery', null, true);		
	...
	if(!empty($googleApiKey)){
		...
		if( is_page_template('template-submit-ads.php') || is_page_template('template-submit-ads-v2.php') || is_page_template('template-edit-ads.php')) {
	...
	wp_enqueue_style( 'fontawesome', get_template_directory_uri() . '/css/fontawesome.css', array(), '1' );
	...
	if(is_admin_bar_showing()) {
		echo "<style type=\"text/css\">.navbar-fixed-top { margin-top: 28px; } </style>";
	}
}