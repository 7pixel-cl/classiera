...
if (!function_exists('classiera_my_category_fields')) {
	function classiera_my_category_fields($tag) {
		...
		if(isset($tag->term_id)){
			...
			$cat_sidebar_adv = isset( $tag_extra_fields[$tag->term_id]['cat_sidebar_adv'] ) ? esc_attr( $tag_extra_fields[$tag->term_id]['cat_sidebar_adv'] ) : '';
			$cat_shortcode_top = isset( $tag_extra_fields[$tag->term_id]['cat_shortcode_top'] ) ? esc_attr( $tag_extra_fields[$tag->term_id]['cat_shortcode_top'] ) : '';
			$cat_shortcode_side = isset( $tag_extra_fields[$tag->term_id]['cat_shortcode_side'] ) ? esc_attr( $tag_extra_fields[$tag->term_id]['cat_shortcode_side'] ) : '';
		}
		?>
	...
	<div class="form-field">	
		<table class="form-table">
			...
			<tr class="form-field">
				<th scope="row" valign="top">
					<label>
						<?php esc_html_e( 'Custom Ad at top', 'classiera' ); ?>
					</label>
				</th>
				<td>
					<textarea name="cat_top_adv" placeholder="<?php esc_attr_e( 'Enter your HTML code here', 'classiera' ); ?>"><?php if(function_exists('classiera_escape')) { classiera_escape($cat_top_adv); } ?></textarea>
					<p class="description wp_cat_desc"><?php esc_html_e( 'You can use Google Ads or any other HTML ads, This will display at top of category page.', 'classiera' ); ?></p>
				</td>
			</tr>
			<tr class="form-field">
				<th scope="row" valign="top">
					<label>
						<?php esc_html_e( 'PHP short-code at top', 'classiera' ); ?>
					</label>
				</th>
				<td>
					<textarea name="cat_shortcode_top" placeholder="<?php esc_attr_e( 'Enter your short-code here', 'classiera' ); ?>"><?php if(function_exists('classiera_escape')) { classiera_escape($cat_shortcode_top); } ?></textarea>
					<p class="description wp_cat_desc"><?php esc_html_e( 'You can use LayerSlider or any PHP short-code', 'classiera' ); ?></p>
				</td>
			</tr>
			<tr class="form-field">
				<th scope="row" valign="top">
					<label>
						<?php esc_html_e( 'Custom Ad at sidebar', 'classiera' ); ?>
					</label>
				</th>
				<td>
					<textarea name="cat_sidebar_adv" placeholder="<?php esc_attr_e( 'Enter your HTML code here', 'classiera' ); ?>"><?php if(function_exists('classiera_escape')) { classiera_escape($cat_sidebar_adv); } ?></textarea>
					<p class="description wp_cat_desc"><?php esc_html_e( 'You can use Google Ads or any other HTML ads, This will display at sidebar of category page.', 'classiera' ); ?></p>
				</td>
			</tr>
			<tr class="form-field">
				<th scope="row" valign="top">
					<label>
						<?php esc_html_e( 'PHP short-code at Sidebar', 'classiera' ); ?>
					</label>
				</th>
				<td>
					<textarea name="cat_shortcode_side" placeholder="<?php esc_attr_e( 'Enter your short-code here', 'classiera' ); ?>"><?php if(function_exists('classiera_escape')) { classiera_escape($cat_shortcode_side); } ?></textarea>
					<p class="description wp_cat_desc"><?php esc_html_e( 'You can use LayerSlider or any PHP short-code', 'classiera' ); ?></p>
				</td>
			</tr>			
			<!--Category Advertisement-->
	...
	function classiera_update_my_category_fields($term_id) {
		if(isset($_POST['taxonomy'])){	
		  if($_POST['taxonomy'] == 'category'):
			...
			$tag_extra_fields[$term_id]['cat_top_adv'] = stripslashes_deep($_POST['cat_top_adv']);
			$tag_extra_fields[$term_id]['cat_sidebar_adv'] = stripslashes_deep($_POST['cat_sidebar_adv']);
			$tag_extra_fields[$term_id]['cat_shortcode_top'] = stripslashes_deep($_POST['cat_shortcode_top']);
			$tag_extra_fields[$term_id]['cat_shortcode_side'] = stripslashes_deep($_POST['cat_shortcode_side']);
...
if (!function_exists('classiera_admin_featuredPlan')) {
	...
	function classiera_admin_featuredPlan() {
		...
		$insert_format = array('%d', '%d', '%d', '%d', '%s','%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s');
		$adminID = 1;	
...
if(!function_exists('classiera_grid_classes')) {
	function classiera_grid_classes(){
		...
		if(is_front_page() || is_page_template('template-homepage-v1.php') || is_page_template('template-homepage-v2.php') || is_page_template('template-homepage-v3.php') || is_page_template('template-homepage-v4.php') || is_page_template('template-homepage-v5.php') || is_page_template('template-homepage-v6.php') || is_page_template('template-homepage-v7.php') || is_page_template('template-landing.php') || is_page_template('template-all-posts.php')){
...
if (!function_exists('classiera_rss_post_thumbnail')){
	function classiera_rss_post_thumbnail($content){
		global $post;
		if(has_post_thumbnail($post->ID)) {						
			$content = '<media:content url="'.get_the_post_thumbnail_url($post->ID).'" medium="image" />'. get_the_content();
		}
		return $content;
	}
	add_filter('the_excerpt_rss', 'classiera_rss_post_thumbnail');
	add_filter('the_content_feed', 'classiera_rss_post_thumbnail');
}
/*==========================
 Classiera : Currency Format Function
 TCF = Thousand Currency Format
 @since classiera 4.0.20
===========================*/
if (!function_exists('classiera_TCF')){
	function classiera_TCF($number){
		if($number > 1000) {
			$x = round($number);
			$x_number_format = number_format($x);
			$x_array = explode(',', $x_number_format);
			$x_parts = array(
				esc_html__( 'K', 'classiera' ),
				esc_html__( 'M', 'classiera' ),
				esc_html__( 'B', 'classiera' ),
				esc_html__( 'T', 'classiera' ),
			);
...
===========================*/
if (!function_exists('classiera_ad_defect')) {
	function classiera_ad_defect($text){	
		$string = str_replace(' ', '', $text);
		if($string == 'no_defect'){
			$returnVal = esc_html__( 'No Defect', 'classiera' );
		}elseif($string == 'minor_defect'){
			$returnVal = esc_html__( 'Minor Defect', 'classiera' );
		}elseif($string == 'major_defect'){
			$returnVal = esc_html__( 'Major Defect', 'classiera' );
		}else{
			$returnVal = '';
		}
		return $returnVal;
