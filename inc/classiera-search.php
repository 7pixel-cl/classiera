...
if (!function_exists('classiera_Select_currency_dropdow')) {
	function classiera_Select_currency_dropdow($tag){
		...
		<select name="post_currency_tag" id="post_currency_tag" class="post_currency_tag form-control form-control-md">
			...
			<option value="BTC" <?php if($tag == 'BTC'){echo "selected";}?>>
				<?php esc_html_e('Bitcoin', 'classiera'); ?>
			</option>			
...
if (!function_exists('classiera_post_price_display')) {
	function classiera_post_price_display($currencyTag, $postPrice){
		...
		$currencyFormat = 'normal';
		if(isset($redux_demo)){
			$classieraCurrencyTag = $redux_demo['classierapostcurrency'];
			$currencyLeftRight = $redux_demo['classiera_currency_left_right'];
			$currencyFormat = $redux_demo['classiera_currency_format'];
		}
		if($currencyFormat == 'custom'){
			$postPrice = classiera_TCF($postPrice);
		}elseif($currencyFormat == 'comma'){
			$postPrice = number_format($postPrice);
		}
		...
		if($currencyLeftRight == 'left'){
			$priceFormat = $currTag.$space.$postPrice;
		}else{
			$priceFormat = $postPrice.$space.$currTag;
		}
		return $priceFormat;
	}
}
