<?php
if(!function_exists('classiera_cart_url')) {
	function classiera_cart_url(){
		$cartURL = '';
...
if(!function_exists('classiera_currency_sign')) {
...
if(!function_exists('classiera_Plans_URL')) {
	function classiera_Plans_URL(){		
		...
		$new_post = classiera_get_template_url('template-submit-ads.php');
		if(empty($new_post)){
			$new_post = classiera_get_template_url('template-submit-ads-v2.php');
		}
...
/*=====================================
	Classiera Pay Per Post Funcation
	Manually
======================================*/
if (!function_exists('classiera_pay_per_post')) {
	function classiera_pay_per_post($productID, $postID, $postTitle, $days){
		$savevalue = array();
		$product_id = $productID; //This is product ID
		$savevalue['product_id'] = $productID;
		$savevalue['post_id'] = $postID;
		$savevalue['post_title'] = $postTitle;
		$savevalue['days_to_expire'] = $days;
		$_SESSION['classiera_user_data'] = $savevalue;
		/*=====================================
		Add data to woocommerce cart
		======================================*/
		$product_id = apply_filters( 'woocommerce_add_to_cart_product_id', absint( $productID ) );
		$quantity = 1;		
...
======================================*/
if (!function_exists('classiera_pay_per_post_id')) {
	add_action('wp_ajax_classiera_pay_per_post_id', 'classiera_pay_per_post_id');
	add_action('wp_ajax_nopriv_classiera_pay_per_post_id', 'classiera_pay_per_post_id'); 
	function classiera_pay_per_post_id(){	
		if(isset($_POST['catID'])){
			if( wp_verify_nonce( $_REQUEST['security'], 'classiera_nonce' ) ){
				...
				$days_to_expire = '';
				$price = '';
				$displayfeaturedtxt = esc_html__( 'Feature this ad for only ', 'classiera' );
...
======================================*/
if (!function_exists('classiera_payperpost')) {
...
/*=====================================
	Classiera Bump Ads
======================================*/
if (!function_exists('classiera_bump_ads')) {
...
if (!function_exists('classiera_implement_woo_ajax')) {
...
if(!function_exists('classiera_insert_data_into_cart')){
 function classiera_insert_data_into_cart($product_name, $values, $cart_item_key ){		
		...
		if(isset($values['wdm_user_custom_data_value']) && count($values['wdm_user_custom_data_value']) > 0){
				...
				if(empty($days_to_expire)){
					if($classieraBumpAds == true){
						...
						if($key == 'plan_time'){
							...
							$return_string .= '<li>'.$echokey.'&nbsp; : &nbsp; '.$val.'</li>';
							$update = esc_html__( 'Please don&rsquo;t update the quantity from here.', 'classiera' );
...
add_action('woocommerce_checkout_create_order_line_item', 'classiera_save_order_meta', 20, 4 );
function classiera_save_order_meta($item, $cart_item_key, $values, $order) {
	global $woocommerce, $wpdb;
	$user_custom_values = $values['wdm_user_custom_data_value'];	
	if(!empty($user_custom_values)){
		foreach($user_custom_values as $key => $val){			
			$item->update_meta_data( $key, $val );
		}            
	}    
}
...
if(!function_exists('classiera_complete_payment_order')){
			...
			if(empty($days_to_expire)){
				if(empty($bump_ads)){
					for($i=0; $i < $quantity ; $i++){ 
						...
						$insert_format = array('%d', '%d', '%d', '%d', '%d', '%s', '%s','%d', '%s', '%s', '%s', '%s', '%s', '%s');
					...
					if( function_exists('classiera_bump_ads_notification')){
						classiera_bump_ads_notification($featured_post_id);
					}
				}
			}else{
				global $post;
