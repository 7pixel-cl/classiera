...
global $user_ID, $user_identity, $user_level, $redux_demo;
$caticoncolor="";
$category_icon_code ="";
$category_icon="";
$category_icon_color="";
...
if ($user_ID){	
	if($_POST){		
		...
		if(isset($_POST['display_name'])){
			...
			update_user_meta($user_ID, 'nickname', $_POST['display_name']);
		}
		if(isset($_POST['gdpr'])){
			$classiera_gdpr = true;
		}else{
		...
		if(isset($password) && !empty($confirmPWD)) {
			...
			if(isset($confirmPWD) && $confirmPWD == $confirmPWD2) {
				if ( $current_user && wp_check_password( $password, $current_user->data->user_pass, $current_user->ID ) ) {
					...
					$message =  esc_html__( 'Current password mismatch', 'classiera' );
				}
			}else{
					...
					<div class="user-ads user-profile-settings">
						...
						<form data-toggle="validator" role="form" method="POST" id="primaryPostForm" action="" enctype="multipart/form-data" autocomplete="off">
							...
							<div class="form-inline row form-inline-margin">
								...
								<div class="form-group col-sm-5">
									<label for="display_name"><?php esc_html_e( 'Display name', 'classiera' ); ?></label>
									<div class="inner-addon">
										<input type="text" id="display_name" name="display_name" class="form-control form-control-sm" placeholder="<?php esc_attr_e( 'Enter your display name', 'classiera' ); ?>" value="<?php echo esc_attr( $user_info->display_name ); ?>">
									</div>
								</div><!--Last name-->
